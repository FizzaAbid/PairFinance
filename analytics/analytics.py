...
while True:
    try:
        psql_engine = create_engine(
            environ["POSTGRESQL_CS"], pool_pre_ping=True, pool_size=10, future=True
        )
        mysql_engine = create_engine(
            environ["MYSQL_CS"], pool_pre_ping=True, pool_size=10, future=True
        )
...
def calculate_max_temperature(
    last_exceution_time, current_start_hour
):  # alternative approach would be to use dictionaries
    psql_metadata.reflect(bind=psql_engine, only=["devices"])
    max_temp_stmt = text(
        "SELECT distinct device_id, max(tmp) as max_temperature,"
        " extract(hour from utc_time) as hour_utc from ("
        ...
        " ORDER BY devices.time ASC)max_tmp group by device_id, hour_utc"
    )
    results = psql_connection.execute(
        max_temp_stmt,
        dict(processed_time=last_exceution_time, hour_start=current_start_hour),
    )
    return results
...
def compute_data_points(last_exceution_time):
    ...
    find_datapoints_stmt = text(
        ...
        " WHERE CAST(devices.time AS INTEGER) > :processed_time"
        " group by devices.device_id,devices.time"
        " ORDER BY devices.time ASC) datapoint group by device_id,hour_utc"
    )
    datapoints = psql_connection.execute(
        find_datapoints_stmt, dict(processed_time=last_exceution_time)
    )
    return datapoints
...
def compute_distance(distances, device_id, hourUtc, location, prior_location):
    ...
    if prior_location:
        distances[device_id][hourUtc] += geopy_distance(
...
def data_aggregation(
    device_query_results, last_exceution_time, current_start_hour
):  # aggregate max_temp, data points, and distance
    distances = {}
    maximum_temp = []
    datapoints = []
    prior_location = None

    maxTemp = calculate_max_temperature(
        last_exceution_time, current_start_hour
    )  # Calculating max temp of each device per hour
    datapoint = compute_data_points(
        last_exceution_time
    )  # calculating data points of each device per hour
    ...
    for row in device_query_results.fetchall():
        ...
        distances, prior_location = compute_distance(
            distances, device_id, hourUtc, location, prior_location
        )
...
while 1:
    ...
    selectDevicesStmt = text(
        "SELECT devices.device_id, devices.temperature, "
    ...
    device_query_results = psql_connection.execute(
        selectDevicesStmt,
        dict(processed_time=last_exceution_time, hour_start=current_start_hour),
    )
    max_temperatures, data_points, distances, last_exceution_time = data_aggregation(
        device_query_results, last_exceution_time, current_start_hour
    )
    insert_results(max_temperatures, data_points, distances)
    current_time_utc = datetime.now(timezone.utc)
    next_hour = current_time_utc.replace(
        hour=current_time_utc.hour + 1, minute=0, second=0, microsecond=0
    )
